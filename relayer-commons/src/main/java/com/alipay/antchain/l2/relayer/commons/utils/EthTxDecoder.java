...
public class EthTxDecoder {
    ...
    public static RawTransaction decode(final String hexTransaction) {
        ...
        return switch (transactionType) {
            case EIP1559 -> decodeEIP1559Transaction(transaction);
            case EIP4844 -> decodeEIP4844Transaction(transaction);
            case EIP2930 -> decodeEIP2930Transaction(transaction);
            default -> decodeLegacyTransaction(transaction);
        };
    }
    ...
    @SneakyThrows
    public static RawTransaction decodeEIP4844Transaction(final byte[] transaction) {
        ...
        var blobSidecarVersion = 0;
        var blobIndex = 1;
        if (outerList.getValues().size() > 4) {
            blobSidecarVersion = ((RlpString) outerList.getValues().get(blobIndex++)).getBytes()[0];
        }
        final List<Blob> blobs = decodeBlobs(((RlpList) outerList.getValues().get(blobIndex++)).getValues());
        final List<Bytes> kzgCommitments =
                decodeBytesList(((RlpList) outerList.getValues().get(blobIndex++)).getValues());
