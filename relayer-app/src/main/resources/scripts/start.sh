...
if [ "$IF_SYS_MODE" == "on" ]; then
  ...
  START_CMD="${JAVA_BIN} ${AGENT_FLAG} -jar -Dlogging.file.path=${CURR_DIR}/../log ${CURR_DIR}/../lib/${JAR_FILE} --spring.config.location=file:${CURR_DIR}/../config/application-prod.yml"
  WORK_DIR="$(
    cd ${CURR_DIR}/..
    pwd
  )"
  ...
  L2_RELAYER_START_CMD="${JAVA_BIN} -XX:InitialRAMPercentage=${JVM_INIT_RAM_PERCENTAGE:-70.0} -XX:MaxRAMPercentage=${JVM_MAX_RAM_PERCENTAGE:-70.0} ${AGENT_FLAG} -jar -Dlogging.file.path=${CURR_DIR}/../log ${CURR_DIR}/../lib/${JAR_FILE} --spring.config.location=file:${CURR_DIR}/../config/application-prod.yml"
