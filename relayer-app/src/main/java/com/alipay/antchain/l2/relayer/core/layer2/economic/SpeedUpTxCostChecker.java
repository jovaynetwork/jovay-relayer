...
@Slf4j
@AllArgsConstructor
public class SpeedUpTxCostChecker implements IRollupCostChecker {
    ...
    public void check(IGasPrice gasPrice) {
        ...
        if (currPrice.compareTo(strategyConfig.getHighEip1559PriceLimit()) <= 0) {
            log.debug("currPrice of speedup tx is in green range, currPrice: {}", currPrice);
            return;
        }
        // red range
        if (reliableTx.getTransactionType() == TransactionTypeEnum.BATCH_COMMIT_TX) {
            ...
            if (count >= strategyConfig.getMaxPendingBatchCount()) {
                log.info("currPrice of batch speedup tx is in red range but pass the economic check, currPrice: {}", currPrice);
                return;
            }
            ...
            if (count >= strategyConfig.getMaxPendingProofCount()) {
                log.info("currPrice of proof speedup tx is in red range but pass the economic check, currPrice: {}", currPrice);
                return;
            }
        }
