...
public class Dispatcher {
    ...
    private List<IDistributedTask> filterTasksInTimeSliceOrNodeOffline(List<IDistributedTask> allTasks, List<ActiveNode> onlineNodes) {
        ...
        for (BizDistributedTask existedTask : bizDistributedTasks) {
            ...
            // if curr running node is online
            if (onlineNodeIdSet.contains(existedTask.getNodeId())) {
                if (!existedTask.ifFinish()) {
                    // if task not finished, remove task from tasks map
                    allTasksMap.remove(existedTask.getTaskType());
                } else {
                    // if task not finished and curr node is still online, set curr running node id to task
                    allTasksMap.get(existedTask.getTaskType()).setNodeId(existedTask.getNodeId());
                }
            }
        }
    ...
    private void roundRobin(List<ActiveNode> nodes, List<IDistributedTask> tasks) {
        ...
        var nodeIdSet = nodes.stream().map(ActiveNode::getNodeId).collect(Collectors.toSet());
        for (int i = 0; i < tasks.size(); ++i) {
            var currTask = tasks.get(i);
            // If task's running node still online and task is BLOCK_POLLING_TASK, continue to dispatch this task to curr node
            // If not, dispatch this task to another node
            // Reason is that blk polling task use local memory to save growing batch chunks,
            // so we decide to try not to switch the execution node of the task.
            if (currTask.getTaskType() != BizTaskTypeEnum.BLOCK_POLLING_TASK || !nodeIdSet.contains(currTask.getNodeId())) {
                currTask.setNodeId(nodes.get(i % nodes.size()).getNodeId());
            }
