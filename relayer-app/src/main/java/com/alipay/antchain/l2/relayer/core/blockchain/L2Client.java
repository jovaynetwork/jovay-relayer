...
public class L2Client extends AbstractWeb3jClient implements L2ClientInterface {
    ...
    public TransactionInfo resendGasFeedTx(String encodedFunc) {
        ...
        return sendTransaction(gasOracleContractAddress, encodedFunc, NopeChecker.INSTANCE);
    }
    ...
    @SneakyThrows
    @WithSpan
    @Retryable(retryFor = {TxSimulateException.class, ClientConnectionException.class, SocketException.class, SocketTimeoutException.class}, backoff = @Backoff(delay = 300), notRecoverable = Exception.class)
    public TransactionInfo updateBatchRollupFee(BigInteger lastBatchDaFee, BigInteger lastBatchExecFee, BigInteger lastBatchByteLength) {
        ...
        var result = sendTx(gasOracleContractAddress, FunctionEncoder.encode(function), createEthCallGasLimitProvider(gasOracleContractAddress, function), NopeChecker.INSTANCE);
