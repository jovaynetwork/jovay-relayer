...
public class MailboxServiceImpl implements IMailboxService {
    ...
    private boolean isBatchReadyForL2MsgProof(BigInteger batchIndex) {
        ...
        if (rollupRepository.getBatchProveRequest(batchIndex, ProveTypeEnum.TEE_PROOF).getState() == BatchProveRequestStateEnum.COMMITTED) {
            return true;
        }
        var zkProveReq = rollupRepository.getBatchProveRequest(batchIndex, ProveTypeEnum.ZK_PROOF);
        return ObjectUtil.isNotNull(zkProveReq) && zkProveReq.getState() == BatchProveRequestStateEnum.COMMITTED;
    }
