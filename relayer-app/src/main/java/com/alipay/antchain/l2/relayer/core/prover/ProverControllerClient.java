...
public class ProverControllerClient {
    ...
    @Retryable(retryFor = RemoteServiceRetryException.class, backoff = @Backoff(delay = 100, multiplier = 2))
    public void notifyChunk(BigInteger batchIndex, long chunkIndex, BigInteger blockStart, BigInteger blockEnd) {
        log.info("notify chunk (batch: {}, chunk: {}, block_start: {}, block_end: {}) to prover controller",
                batchIndex, chunkIndex, blockStart, blockEnd);
        var status = this.stub.notifyChunk(ChunkInfo.newBuilder()
                .setBatchId(batchIndex.longValue())
                .setChunkId(chunkIndex)
                .setBlockStart(blockStart.longValue())
                .setBlockEnd(blockEnd.longValue() + 1)
                .build()
        );
        if (status.getErrorCode() != L2ErrorCode.L2_OK) {
            if (RemoteServiceUtils.isL2StatusRetryable(status)) {
                throw new RemoteServiceRetryException(
                        status.getErrorCode(), status.getErrorMessage(),
                        "failed to notify chunk (batch: {}, chunk: {}, block_start: {}, block_end: {})",
                        batchIndex, chunkIndex, blockStart, blockEnd
                );
            }
            throw new CallRemoteServiceFailedException(
