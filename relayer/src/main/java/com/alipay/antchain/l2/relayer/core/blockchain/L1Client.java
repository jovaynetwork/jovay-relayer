...
public class L1Client extends AbstractWeb3jClient implements L1ClientInterface {
    ...
    public TransactionInfo commitBatch(BatchWrapper batchWrapper, BatchHeader parentBatchHeader) throws L2RelayerException {
        ...
        return sendBlobTransaction(this.rollupContractAddress, function, batchWrapper.getBatch().getEthBlobs());
    }
    ...
    public TransactionInfo verifyBatch(ProveTypeEnum proveType, BatchWrapper batchWrapper, byte[] proof) throws L2RelayerException {
        log.info("start sending tx to verify batch#{} with retry {}", batchWrapper.getBatchIndex(),
                ObjectUtil.isNull(RetrySynchronizationManager.getContext()) ? 0 : RetrySynchronizationManager.getContext().getRetryCount());
        selfReportMetric.recordStart(RollupMetricRecord.createCommitProofRecord(proveType, batchWrapper.getBatch().getBatchIndex()));
        // 1. check
        // 2. create func
        Function function = new Function(
                FUNC_VERIFYBATCH, // function name
                Arrays.asList(new Uint8(proveType.getRollupProofNum()),
                        ...
                        new DynamicBytes(proof)), // inputs
                Collections.emptyList()// outputs
        );

        return sendTransaction(this.rollupContractAddress, function);
    }
    ...
    public TransactionInfo resendRollupTx(String encodedFunc) {
        ...
        return sendTransaction(this.rollupContractAddress, encodedFunc);
    }
    ...
    public TransactionInfo resendRollupTx(Transaction4844 transaction4844) {
        ...
        return sendBlobTransaction(this.rollupContractAddress, transaction4844.getData(), new EthBlobs(transaction4844.getBlobs().orElse(new ArrayList<>())));
    }
