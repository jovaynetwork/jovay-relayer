...
public class RollupServiceImpl implements IRollupService {
            ...
            transactionTemplate.execute(
                    new TransactionCallbackWithoutResult() {
                        ...
                        protected void doInTransactionWithoutResult(TransactionStatus status) {
                            ...
                            BigInteger batchThisBlockBelongsTo = rollupAggregator.process(blockTrace, currHeight);
    ...
    public void commitL2TeeProof() {
        ...
        if (lastCommittedBatchIdx.compareTo(latestVerifiedBatchIdxOnChain) <= 0) {
            log.debug("proof commit task waiting for batch#{} to be committed", latestVerifiedBatchIdxOnChain.add(BigInteger.ONE));
            return;
        }
    ...
    public void commitL2ZkProof() {
        ...
        BigInteger latestVerifiedBatchIdxOnChain = l1Client.lastZkVerifiedBatch();
        if (ObjectUtil.isNull(latestVerifiedBatchIdxOnChain)) {
            throw new CommitL2BatchTeeProofException("null result from calling lastTeeVerifiedBatch of rollup contract");
        }
        BigInteger nextBatchIdxToCommitZkProof = latestVerifiedBatchIdxOnChain.add(BigInteger.ONE);

        BatchProveRequestDO request = rollupRepository.getBatchProveRequest(nextBatchIdxToCommitZkProof, ProveTypeEnum.ZK_PROOF);
        if (ObjectUtil.isNull(request)) {
            log.debug("next batch {} has not generated for now, so skip this zk proof process...", nextBatchIdxToCommitZkProof);
            return;
        }
        if (ObjectUtil.isEmpty(request.getProof()) || request.getState() == BatchProveRequestStateEnum.PENDING) {
        ...
        BatchWrapper batchWrapper = rollupRepository.getBatch(nextBatchIdxToCommitZkProof);
        if (ObjectUtil.isNull(batchWrapper)) {
            throw new CommitL2BatchTeeProofException("batch for zk proof {} not found", nextBatchIdxToCommitZkProof);
        }
        ...
        try {
            transactionInfo = l1Client.verifyBatch(ProveTypeEnum.ZK_PROOF, batchWrapper, request.getProof());
    ...
    private void commitNextTeeProof(BigInteger nextBatchIdxToCommitTeeProof) {
        ...
        TransactionInfo transactionInfo;
        try {
            transactionInfo = l1Client.verifyBatch(ProveTypeEnum.TEE_PROOF, batchWrapper, request.getProof());
        } catch (L1ContractWarnException e) {
